  ...
  <div ref="input" class="vcp-progress-hover" @click="seek">
    <div ref="container" class="vcp-progress">
      ...
      <div class="vcp-progress-played" :style="{width: progress + '%'}">
        <div ref="thumb" class="thumb-drag" @touchstart="startDrag" @mousedown="startDrag"></div>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    showTime(e) {
      ...
      const duration = this.$player.getDuration()
      this.previewProgress = (left / maxVal * 100).toFixed(2)
    ...
    seek(e) {
      ...
      const left = e.pageX - _clientRect.left
      const maxVal = e.currentTarget.offsetWidth
      this.progress = (left / maxVal * 100).toFixed(2)
      const duration = this.$player.getDuration()
    ...
    initDrag(e) {
      ...
      const move = function(ev) {
        ...
        if (newCoor) {
          ...
          self.progress = (left / maxVal * 100).toFixed(2)
